...
class MeMCeNNLayer(nn.Module):
    ...
    def forward(self, x,v):
        ...
        for step in range(self.IterNum):
                ...
                vdot=0
                Out=self.Aa*torch.sinh(torch.tensor(vm/self.sigma_off))*torch.exp(-torch.square(self.y_off/x))*torch.exp(1/(1+self.beta*p))*self.stepfunc(-vm)+self.Bb*torch.sinh(vm/self.sigma_on)*torch.exp(-torch.square(x/self.y_on))*torch.exp(p/self.sigma_p)*self.stepfunc(vm) 
                
                Out=self.NonLin(x)
                
                InputCoupling=self.B(x) 
                OutputCoupling=self.A(Out) 
...
class CeNNLayer(nn.Module):
    def __init__(self, InDepth=1, OutDepth=1,TimeStep=0.1,IterNum=100):
        ...
        self.A= nn.Conv2d(OutDepth, OutDepth, kernel_size=3, padding=1)/10.0
