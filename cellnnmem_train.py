...
generator = CellNetMem.CellNN(3, img_data.num_classes).cuda()

gen_optimizer = torch.optim.Adam(list(generator.parameters())+generator.Memparams, lr=lr)


# load pretrained model if it is there
file_model = './cellnetmem.pkl'
...
file_loss = open('./cellnetmem_loss', 'w')


# make the result directory
if not os.path.exists('./result_cellmem/'):
    os.makedirs('./result_cellmem/')
...
for epoch in range(epochs):
    for idx_batch, (imagergb, labelmask, labelrgb) in enumerate(img_batch):
        ...
        if idx_batch % 400 == 0:
            ...
            v_utils.save_image(x[:4,:,:,:].cpu().data, "./result_cellmem/original_image_{}_{}.png".format(epoch, idx_batch))
            v_utils.save_image(labelrgb[:4,:,:,:].float().cpu().data, "./result_cellmem/label_image_{}_{}.png".format(epoch, idx_batch))
            ...
            if args.losstype == "segment":
                ...
                v_utils.save_image(y_threshed[:4,:,:,:], "./result_cellmem/gen_image_{}_{}.png".format(epoch, idx_batch))
            else:
                v_utils.save_image(y[:4,:,:,:].cpu().data, "./result_cellmem/gen_image_{}_{}.png".format(epoch, idx_batch))

            # finally checkpoint this file to disk
    if epoch % 20 == 0:
            torch.save(generator, './cellnetmem_'+str(epoch)+'.pkl')
