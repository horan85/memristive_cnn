...
parser.add_argument("--datadir", type=str, default="/home/storage/cityscapes/", help="directory the Cityscapes dataset is in")
...
generator = CellNetTao.CellNN(3, img_data.num_classes).cuda()

gen_optimizer = torch.optim.Adam(list(generator.parameters()), lr=lr)


# load pretrained model if it is there
file_model = './cellnettao.pkl'
...
file_loss = open('./cellnettao_loss', 'w')


# make the result directory
if not os.path.exists('./result_celltao/'):
    os.makedirs('./result_celltao/')
...
for epoch in range(epochs):
    for idx_batch, (imagergb, labelmask, labelrgb) in enumerate(img_batch):
        ...
        x = Variable(imagergb*0.2).cuda(0)
        ...
        if idx_batch % 400 == 0:
            ...
            v_utils.save_image(x[:4,:,:,:].cpu().data, "./result_celltao/original_image_{}_{}.png".format(epoch, idx_batch))
            v_utils.save_image(labelrgb[:4,:,:,:].float().cpu().data, "./result_celltao/label_image_{}_{}.png".format(epoch, idx_batch))
            ...
            if args.losstype == "segment":
                ...
                v_utils.save_image(y_threshed[:4,:,:,:], "./result_celltao/gen_image_{}_{}.png".format(epoch, idx_batch))
            else:
                v_utils.save_image(y[:4,:,:,:].cpu().data, "./result_celltao/gen_image_{}_{}.png".format(epoch, idx_batch))

            # finally checkpoint this file to disk
    if epoch % 20 == 0:
            torch.save(generator, './cellnettao_'+str(epoch)+'.pkl')
